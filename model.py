...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self,d_model : int ,seq_len : int , dropout : float) -> None:
        ...
        divider = torch.exp(torch.arange(0,d_model,2).float()*(-math.log(100000.0)/d_model))

        pe[:,0::2] = torch.sin(position*divider)
        pe[:,1::2] = torch.cos(position*divider)

        pe = pe.unsqueeze(0) #1,seq_len,d_model (for batches)
...
class LayerNormalization(nn.Module):
    ...
    def forward(self,x):
        ...
        x = self.aplha * (x-mean)/(std+self.eps) + self.bias
        return x
...
class Transformer(nn.Module):
    ...
    def __init__(self,encoder : Encoder, decoder : Decoder, src_embed : inputEmb,tgt_embed : inputEmb,src_pos : PositionalEncoding, tgt_pos : PositionalEncoding, projection_layer : ProjectionLayer):
...
def build_transformer(src_vocab_size : int, tgt_vocab_size : int, src_seq_len : int, tgt_seq_len : int,d_model : int = 512, N : int = 6, h : int = 8, dropout : float = 0.1, d_ff : int = 2048) -> Transformer:
    ...
    src_embed = inputEmb(d_model,src_vocab_size)
    tgt_embed = inputEmb(d_model,tgt_vocab_size)
    ...
    for i in range(N):
        ...
        decoder_cross_attention_blok = MultiHeadAttention(d_model,h,dropout)
        feed_forward_block = Feedforward(d_model,d_ff,dropout)
        decoder_block = EncoderBlock(decoder_self_attention_block,decoder_cross_attention_blok,feed_forward_block,dropout)
